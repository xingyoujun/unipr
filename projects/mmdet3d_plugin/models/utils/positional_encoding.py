...
class LearnedPositionalEncoding3D(BaseModule):
    ...
    def __init__(self,
        ...
        self.u_embed = nn.Embedding(u_num_embed, num_feats)
        self.v_embed = nn.Embedding(v_num_embed, num_feats)
        self.num_feats = num_feats
        self.u_num_embed = u_num_embed
        self.v_num_embed = v_num_embed

        self.d_embed = nn.Embedding(d_num_embed, num_feats)
        self.d_num_embed = d_num_embed
    ...
    def forward(self, mask):
        ...
        """
        dl, vl, ul = mask.shape[-3:]
        u = torch.arange(ul, device=mask.device)
        v = torch.arange(vl, device=mask.device)
        d = torch.arange(dl, device=mask.device)

        u_embed = self.u_embed(u)
        v_embed = self.v_embed(v)
        d_embed = self.d_embed(d)
        pos = torch.cat((u_embed.unsqueeze(0).unsqueeze(0).repeat(dl, vl, 1, 1), 
                        v_embed.unsqueeze(0).unsqueeze(2).repeat(dl, 1, ul, 1),
                        d_embed.unsqueeze(1).unsqueeze(1).repeat(1, vl, ul, 1),),dim=-1)
        pos = self.proj(pos)
    ...
    def __repr__(self):
        ...
        repr_str = self.__class__.__name__
        repr_str += f'(num_feats={self.num_feats}, '
        repr_str += f'u_num_embed={self.u_num_embed}, '
        repr_str += f'v_num_embed={self.v_num_embed}, '
        repr_str += f'd_num_embed={self.d_num_embed})'
        return repr_str
...
class CustomPositionalEncoding(BaseModule):
    ...
    def __init__(self,
        ...
        self.proj = nn.Linear(self.num_feats[0]*3, self.num_feats[0]*2, bias=False)

    def forward(self, bs, device, ignore_axis='z'):
        if ignore_axis == 'h':
            h_embed = torch.zeros(1, 1, self.num_feats[0], device=device).repeat(self.z, self.w, 1) # w, z, d
            w_embed = self.w_embed(torch.arange(self.w, device=device))
            w_embed = w_embed.reshape(1, self.w, -1).repeat(self.z, 1, 1)
            z_embed = self.z_embed(torch.arange(self.z, device=device))
            z_embed = z_embed.reshape(self.z, 1, -1).repeat(1, self.w, 1)
        elif ignore_axis == 'w':
        ...
        pos = torch.cat(
            (w_embed, h_embed, z_embed), dim=-1).flatten(0, 1).unsqueeze(0).repeat(
                bs, 1, 1)
        pos = self.proj(pos)
