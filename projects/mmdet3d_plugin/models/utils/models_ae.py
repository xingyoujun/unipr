...
class AutoEncoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.latent = nn.Embedding(1, dim)
        self.point_latent = nn.Embedding(self.num_latents, dim)
        
        self.embeding_encoder_layers = nn.ModuleList([])
        self.embeding_decoder_layers = nn.ModuleList([])
        for i in range(2):
            self.embeding_encoder_layers.append(nn.ModuleList([
                get_latent_attn(**cache_args),
                get_latent_ff(**cache_args)
            ]))

        for i in range(6):
            self.embeding_decoder_layers.append(nn.ModuleList([
                get_latent_attn(**cache_args),
                get_latent_ff(**cache_args)
            ]))
    ...
    def encode(self, pc):
        ...
        emb_latents = self.latent.weight.to(x.dtype)
        emb_latents = emb_latents.unsqueeze(0).repeat(B, 1, 1)
        for emb_cross_attn, emb_cross_ff in self.embeding_encoder_layers:
            emb_latents = emb_cross_attn(emb_latents, context = x, mask = None) + emb_latents
            emb_latents = emb_cross_ff(emb_latents) + emb_latents

        return x, emb_latents
    ...
    def decode_emb(self, emb_latents, queries):
        point_latents = self.point_latent.weight.to(emb_latents.dtype)
        point_latents = point_latents.unsqueeze(0).repeat(emb_latents.shape[0], 1, 1)
        for pot_cross_attn, pot_cross_ff in self.embeding_decoder_layers:
            point_latents = pot_cross_attn(point_latents, context = emb_latents, mask = None) + point_latents
            point_latents = pot_cross_ff(point_latents) + point_latents

        o_pot = self.decode(point_latents, queries).squeeze(-1)

        return {'logits': o_pot}
    
    def forward(self, pc, queries):
        x, emb_latents = self.encode(pc)

        point_latents = self.point_latent.weight.to(x.dtype)
        point_latents = point_latents.unsqueeze(0).repeat(emb_latents.shape[0], 1, 1)
        for pot_cross_attn, pot_cross_ff in self.embeding_decoder_layers:
            point_latents = pot_cross_attn(point_latents, context = emb_latents, mask = None) + point_latents
            point_latents = pot_cross_ff(point_latents) + point_latents
        ...
        return {'logits': o_pot, 'logits_pot': o_pot}
...
class KLAutoEncoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.latent = nn.Embedding(1, dim)
        self.point_latent = nn.Embedding(self.num_latents, latent_dim)
        ...
        get_latent_attn = lambda: PreNorm(latent_dim, Attention(latent_dim, heads = heads, dim_head = dim_head, drop_path_rate=0.1))
        get_latent_ff = lambda: PreNorm(latent_dim, FeedForward(latent_dim, drop_path_rate=0.1))
        get_latent_attn, get_latent_ff = map(cache_fn, (get_latent_attn, get_latent_ff))
    ...
    def forward(self, pc, queries):
        ...
        point_latents = self.point_latent.weight.to(x.dtype)
        point_latents = point_latents.unsqueeze(0).repeat(x.shape[0], 1, 1)
        for pot_cross_attn, pot_cross_ff in self.embeding_decoder_layers:
            point_latents = pot_cross_attn(point_latents, context = x, mask = None) + point_latents
            point_latents = pot_cross_ff(point_latents) + point_latents
        x = point_latents
...
def kl_d256_m256_l256(N=2048):
    return create_autoencoder(dim=256, M=256, latent_dim=256, N=N, determinisitc=False)

def kl_d256_m512_l256(N=2048):
    return create_autoencoder(dim=256, M=512, latent_dim=256, N=N, determinisitc=False)

def kl_d256_m512_l64(N=2048):
    return create_autoencoder(dim=256, M=512, latent_dim=64, N=N, determinisitc=False)
